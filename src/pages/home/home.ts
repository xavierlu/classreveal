...
export class HomePage {
        ...
        if (this.profileData.getUsersSchool() === '') {
          ...
          if (!this.profileData.canEdit(periodNumber)) {
            let alert = this
              .alertCtrl
              .create({
                title: 'Stop stalking your crush. You may only change your class once per session.',
                buttons: [
                  {
            ...
            window
              .localStorage
              .setItem('current-modifying-peroid', JSON.stringify(data));
            this
              .navCtrl
              .push(ListPage);
          }
  ...
  removeTeacher(periodNumber : number) {
    var jsonData = JSON.parse(window.localStorage.getItem('current-modifying-peroid'));
    let alert = this
      .alertCtrl
      .create({
        ...
        buttons: [
          {
            ...
            handler: data => {
              this
                .profileData
                .updateTeacher("", periodNumber, String(this.profileData.getPeriod(+ periodNumber)));
            }
          }
        ]
      });
  ...
  viewClass(periodNumber : number) : void {
    ...
    window
      .localStorage
      .setItem('current-viewing-period', JSON.stringify(data));
    this
      .navCtrl
      .push(ListStudent);
  }
